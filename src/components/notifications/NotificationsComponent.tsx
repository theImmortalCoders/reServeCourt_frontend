        ...
        try {
            ...
            if (result === 200) {
                ...
                setTimeout(() => {
                    ...
                    setRemovedNotifications(prev => prev.filter(id => id !== notificationId));
                }, 300);
            } else {
    ...
    return (
        <div className="fixed bottom-0 right-0 m-6 space-y-2 min-w-48 max-w-80 text-wrap text-sm xs:text-base">
            {messages.map((message) => (
                <div key={message.id} className={`flex flex-col justify-center items-center p-4 bg-white rounded shadow-md shadow-lightGrey transition-all duration-300 ${removedNotifications.includes(message.id) ? 'opacity-0 transform scale-0.9' : ''}`}>
